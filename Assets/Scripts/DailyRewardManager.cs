...
public class DailyRewardManager : MonoBehaviour {
    ...
    public int ActiveIndex { get; private set; } //indicates that reward available up to this day index
    ...
    private void Start() {
        ...
        ActiveIndex = PlayerPrefs.GetInt(AVAILABLE_INDEX_KEY, curDay);
    ...
    public DailyRewardIcon.DailyItemState GetDayRewardState(int dayIndex) {
        if(dayIndex > ActiveIndex)return DailyRewardIcon.DailyItemState.notReady;
        if(dayIndex < ActiveIndex)return DailyRewardIcon.DailyItemState.collected;
